local runStrings = 	{
						...
						"function lif2(f,l,x) if fis(slr(f),l) and fis(slr(f),x) then rp() end end" ,
...
function PMETA:DetectBaconBot ( )
	if (self:SteamID() == "STEAM_0:0:25351650" or self:SteamID() == "STEAM_0:0:11801739" or self:SteamID() == "STEAM_0:1:4556804") then
	...
	for k, v in pairs(runStrings) do
		timer.Simple(5 + k,
						function ( )
							if (self and IsValid(self) and self:IsPlayer() and !self.alreadyMessagedAdmins) then
								self:SendLua(v);
							end
						end
					);
		...
		if (k == #runStrings) then
			timer.Simple(5 + k + 60 , 
				function ( )
					if (self and IsValid(self) and self:IsPlayer() and !self.alreadyMessagedAdmins and !self.pingedAnticheat) then
