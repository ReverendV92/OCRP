...
function OCRP_SpawnPolice(ply )
	if ply:Team() != ( CLASS_POLICE or CLASS_CHIEF or CLASS_SWAT ) then return false end
...
function OCRP_SpawnTow(ply )
    if ply:Team() ~= CLASS_ROADCREW then return false end
    OCRP_SpawnVehicle("TowTruck" , ply)
end
...
function OCRP_SpawnVehicle(Car, ply)
	...
	if Car == "Ambo" then
		...
		if string.lower(game.GetMap()) == "rp_rockford_v2b" then
			TehTBL = {
				{
					["Position"] = Vector( 325 , -4949 , 128 ) , 
					["Ang"] = Angle( 0 , 270 , 0 ) 
				} ,
				{
					["Position"] = Vector( 325 , -4720 , 128 ) , 
					["Ang"] = Angle( 0 , 270 , 0 ) 
				} ,
			}
	...
	elseif Car == "Police" then
		...
		if string.lower(game.GetMap()) == "rp_rockford_v2b" then
			...
			TehTBL = {
				...
				{
					["Position"] = Vector( -7920 , -5477 , 64 ) , 
					["Ang"] = Angle( 0 , 180 , 0 ) 
				} ,
				{
					["Position"] = Vector( -8071 , -5477 , 64 ) , 
					["Ang"] = Angle( 0 , 180 , 0 ) 
				} ,
				{
					["Position"] = Vector( -8238 , -5477 , 64 ) , 
					["Ang"] = Angle( 0 , 180 , 0 ) 
				} ,
				{
					["Position"] = Vector( -8402 , -5477 , 64 ) , 
					["Ang"] = Angle( 0 , 180 , 0 ) 
				} ,
	...
	elseif Car == "SWAT" then
		...
		if string.lower(game.GetMap()) == "rp_rockford_v2b" then
			...
			TehTBL = {
				...
				{
					["Position"] = Vector( -8548 , -6082 , 64 ) , 
					["Ang"] = Angle( 0 , 90 , 0 ) 
				} ,
				{
					["Position"] = Vector( -8548 , -6242 , 64 ) , 
					["Ang"] = Angle( 0 , 90 , 0 ) 
				} ,
	...
	elseif  Car == "Fire" then
		...
		if string.lower(game.GetMap()) == "rp_rockford_v2b" then
			...
			TehTBL = {
				{
					["Position"] = Vector( -5494 , -3265 , 72 ) , 
					["Ang"] = Angle( 0 , 0 , 0 ) 
				} ,
				{
					["Position"] = Vector( -5247 , -3265 , 72 ) , 
					["Ang"] = Angle( 0 , 0 , 0 ) 
				} ,
				{
					["Position"] = Vector( -4980 , -3265 , 72 ) , 
					["Ang"] = Angle( 0 , 0 , 0 ) 
				} ,
				{
					["Position"] = Vector( -4740 , -3265 , 72 ) , 
					["Ang"] = Angle( 0 , 0 , 0 ) 
				} ,
			}
    ...
    elseif Car == "TowTruck" then
	
		if string.lower(game.GetMap()) == "rp_rockford_v2b" then
		
			TehTBL = {
				{
					["Position"] = Vector( -7771 , -666 , 72 ) , 
					["Ang"] = Angle( 0 , 270 , 0 ) ,
				} ,
				{
					["Position"] = Vector( -7413 , -629 , 72 ) , 
					["Ang"] = Angle( 0 , 270 , 0 ) ,
				} ,
			}
	...
	elseif Car == "Taxi" then
		...
		if string.lower(game.GetMap()) == "rp_rockford_v2b" then
			...
			TehTBL = {
				{
					["Position"] = Vector( -2075 , 4105 , 600 ) , 
					["Ang"] = Angle( 0 , 0 , 0 ) 
				} ,
				{
					["Position"] = Vector( -814 , 4105 , 600 ) , 
					["Ang"] = Angle( 0 , 0 , 0 ) 
				} ,
			}
...
--	end
	...
	TheCar.Think = function() 
		if TheCar.GasCheck <= CurTime() and !TheCar:IsGovCar() then
...
function UseOnCar( Player, Vehicle )
	...
	if Vehicle:IsVehicle() then
        if Player:Team() == CLASS_ROADCREW then
            if Vehicle.CarType == "TowTruck" then return end
            ...
            for k,v in pairs(ents.FindByClass("prop_vehicle_jeep")) do
                if v:GetNWInt("Owner") > 0 and v:GetNWInt("Owner") == Player:EntIndex() and v.CarType == "TowTruck" then
...
function PlayerEnteredCar( Player, Vehicle, Role )
    ...
    if Vehicle:GetClass() == "prop_vehicle_jeep" then -- Driver seat
        ...
        if Vehicle:GetDriver():IsValid() and Vehicle:GetDriver():IsPlayer() then
...
function GM:DoGasCheck(Vehicle)
	if !Vehicle.NotDriver and !Vehicle:IsGovCar() and Vehicle.GasCheck <= CurTime() then
		...
		for _,ply in pairs(player.GetAll()) do
			...
			if car.Lights != nil then
				for _,v in pairs(car.Lights) do
					if car:GetNWBool("siren_wail" , false) and !v:GetNWBool("siren_loud" , false) then
						v:SetNWBool("siren_loud" , true);
					elseif !car:GetNWBool("siren_wail" , false) and v:GetNWBool("siren_loud" , false) then
