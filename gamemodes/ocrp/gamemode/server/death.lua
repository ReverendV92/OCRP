...
function GM:DoPlayerDeath( ply, attacker, dmginfo )
	...
	elseif attacker:IsPlayer() and attacker:Alive() then
		if attacker:GetActiveWeapon():GetClass() != "ocrp_hands" and attacker:GetActiveWeapon():GetClass() != "weapon_bat_ocrp" and attacker:GetActiveWeapon():GetClass() != "ocrp_baton" then
			if attacker:GetActiveWeapon():GetClass() == "ocrp_taser" then
	...
	if attacker:IsPlayer() and attacker:GetActiveWeapon() and attacker:GetActiveWeapon():GetClass() then
		if ply.KOInfo and ply.KOInfo.Death == false then
	...
	if ( attacker:IsValid() and attacker:IsPlayer() ) then
 	...
 	for item,data in pairs(GAMEMODE.OCRP_Items) do
		if data.Weapondata != nil then
			if ply:GetActiveWeapon() != nil and data.Weapondata.Weapon == ply:GetActiveWeapon():GetClass() and ply:HasItem(item) and ply:Team() == CLASS_CITIZEN and !data.Weapondata.DontDrop then
				ply:DropItem(item)
				break
			end
		end
...
function GM:PlayerDeathThink( ply )
	...
	if (  ply.KOInfo.Wait != nil and ply.KOInfo.Wait > CurTime() ) then return end
	if !ply:Alive() then
		if ply.KOInfo.Death then 
