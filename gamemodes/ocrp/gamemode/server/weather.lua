...
function GM.calculateWeather ( ) 
	...
	if (GAMEMODE.NextTempChange <= 0) then
		...
		if (GAMEMODE.CurrentTime < DAWN_START or GAMEMODE.CurrentTime > DUSK_START) then
			ourTable = ourTable[2];
		else
			ourTable = ourTable[1];
		end
...
function GM.handleSnow ( )
	...
	if (!GAMEMODE.SnowOnGround and GetGlobalInt("temp" , 35) <= 32 and storming and GAMEMODE.LastWeatherChange + 20 <= CurTime()) then
