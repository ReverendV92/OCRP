...
net.Receive("OCRP_Buy_Property" , function(len, ply)
	if !ply:NearNPC( "Realtor" ) then return false end
...
function OCRP_Set_Permissions(ply,door,Org,Bud,Gov)
	if door.UnOwnable != nil and ply:Team() == CLASS_CITIZEN then return end 
...
function OCRP_Has_Permission(ply,obj)
	...
	if permtbl["Goverment"] and table.HasValue(etable, ply:Team()) then
		return true
	end
	if permtbl["Org"] and owner != nil then
...
function OCRP_Toggle_Lock(ply,door)
	if OCRP_Has_Permission(ply,door) then
		if door:GetNWBool("UnLocked") then
			...
			if door.PadLock != nil and ply == door.PadLock.Owner then
