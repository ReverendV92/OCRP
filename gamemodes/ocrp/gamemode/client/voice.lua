...
local function thinkRadioStatic ( )
	if (GAMEMODE.PlayStatic) then
		...
		if (!GAMEMODE.NextStaticPlay or GAMEMODE.NextStaticPlay < CurTime()) then
...
local function PlayerStartVoice ( ply ) 
	...
	if (ply:GetPos():Distance(LocalPlayer():GetPos()) > (ChatRadius_Local + 50) and ply:Team() != CLASS_CITIZEN and ply:Team() != CLASS_MAYOR) then
...
local function monitorKeyPress_WalkieTalkie ( )
 ...
 if TYPING then return; end
  ...
  if LocalPlayer():Team() == CLASS_CITIZEN or LocalPlayer():Team() == CLASS_TAXI or LocalPlayer():Team() == CLASS_ROADCREW then return end
