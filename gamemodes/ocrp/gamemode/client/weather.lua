...
local function weatherThink ( )
	...
	local shouldSlush = currentTemp < 37 and currentTemp > 34;
	...
	// Transition noises
	if (currentWeather != lastWeather and LocalPlayer():IsOutside()) then
		local currentStormTable = GAMEMODE.StormTable[ourShouldVal][currentWeather];
		
		if (currentStormTable and currentStormTable["FADE_IN_SOUND"]) then
			if (!lastStormTable or !lastStormTable["LOOP_SOUND"]) then
		...
		elseif (lastStormTable and  lastStormTable["LOOP_SOUND"]) then
	...
	// Precipitation
	if (lastStormTable and lastStormTable["PRECIP_EFFECT"] and nextPrecip <= CurTime()) then	
