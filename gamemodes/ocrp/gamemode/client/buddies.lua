...
function CL_SprintDecay(ply, data)
	...
	if ply:KeyDown(IN_SPEED) and ply:OnGround() and !GetForceWalk() then
		...
		--	data:SetForwardSpeed(data:GetForwardSpeed())
			if CL_GetEnergy() > 0 and CL_GetChargeInt() <= CurTime()  then
				CL_SetEnergy(CL_GetEnergy() - 1)
				LocalPlayer().ChargeInt = CurTime() + 0.05
			end
		end
	else
		if CL_GetEnergy() < 100 and CL_GetChargeInt() <= CurTime() then
			CL_SetEnergy(CL_GetEnergy() + 1)
			LocalPlayer().ChargeInt = CurTime() + 1
		end
	end
	if CL_GetEnergy() > 0 and !LocalPlayer().CanSprint then
		LocalPlayer().CanSprint = true
	elseif CL_GetEnergy() <= 0 and LocalPlayer().CanSprint  then
