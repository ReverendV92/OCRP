...
if CLIENT then
	...
	SWEP.Contact = ""
	SWEP.Purpose = "Put out fires fast"
...
function SWEP:PrimaryAttack()
	...
	if CLIENT or (game.SinglePlayer() and SERVER) then		
		...
		util.Effect('ocrp_fire_hose_water', ED);
	end
	...
	if SERVER then
		...
		if timer.Exists("ocrp_fire_hose_water_sound_stop") then
			timer.Destroy("ocrp_fire_hose_water_sound_stop")
		end
		timer.Create("ocrp_fire_hose_water_sound_stop", .2, 1, function()
