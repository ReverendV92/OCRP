				...
				timer.Simple(1,function() 
						...
						local Random = math.random( 0, 100 )
							if Random > Chance && Random < 60 then
								...
								self.Owner:SelectWeapon("ocrp_hands")
								self:Remove()
...
function SWEP:EmptyClip()
	if SERVER then
		...
		timer.Simple(5,function() 
			...
			if self.Owner:HasWeapon("ocrp_lockpick") && self.Owner:GetActiveWeapon():GetClass() == "ocrp_lockpick" then 
				if self.Owner:IsPlayer() then
					self.Owner:GiveItem("item_lockpick",1) 
					self.Owner:StripWeapon("ocrp_lockpick") 
				end
			end 
		end)
	end
