...
AddGear(" Give Hands/Keys", "Re-equips target with hands and keys.", false,
	function ( Player, Trace )
			if IsValid(Trace.Entity) and Trace.Entity:IsPlayer() then
				if Trace.Entity:HasWeapon("ocrp_hands") then
					Trace.Entity:StripWeapon("ocrp_hands")
				elseif Trace.Entity:HasWeapon("ocrp_keys") then
					Trace.Entity:StripWeapon("ocrp_keys")
				end
				timer.Simple(.5, function ( )
					Trace.Entity:Give("ocrp_hands")
					Trace.Entity:Give("ocrp_keys")
					--end
				end)
			end
	end
...
AddGear(" Handcuff Player", "Aim at a player to handcuff him.", false,
	function ( Player, Trace )
			if IsValid(Trace.Entity) and Trace.Entity:IsPlayer() then
				...
				Trace.Entity:SelectWeapon("ocrp_hands")
