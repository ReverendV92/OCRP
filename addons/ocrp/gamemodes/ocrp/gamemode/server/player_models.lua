...
function PMETA:SetJobModel()
	...
	elseif t == CLASS_ROADCREW then
		path = path .. "towtruck/10/"
	elseif t == CLASS_SWAT then
		path = path .. "swat/10/"
	elseif t == CLASS_MEDIC then
		path = path .. "paramedic/10/"
	elseif t == CLASS_TAXI then
		path = path .. "busdriver/10/"
	end
	if t != CLASS_CITIZEN then
		self:SetModel(path .. mdl)
	end
		-- local path = "models/dynamicrp/characters/"
	-- if t == CLASS_POLICE then
		-- path = path .. "police/"
    -- elseif t == CLASS_CHIEF then
        -- path = path .. "police/"
