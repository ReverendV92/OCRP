...
function SV_SellItem(ply,item,ShopId,amount) 
	...
	if ply:HasItem(item,tonumber(amount)) and bool then
...
function META:PriceItem(price,desc)
	...
	for _,ent in pairs(ents.FindByClass("item_base")) do 
		if self:GetNWInt("Owner") == ent:GetNWInt("Owner") and ent.price != nil and ent.desc != nil then
			num = num + 1
		end
	end
	...
	for _,ply in pairs(player.GetAll()) do
		if ply:IsValid() then
			local amt = 0
			if self.Amount != nil and tonumber(self.Amount) > 1 then
				amt = self.Amount
			else
				amt = 1
			end		
