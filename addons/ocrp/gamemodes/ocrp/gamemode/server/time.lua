...
local function buildLightTable ( )
	...
	for x = 1 , DAY_LENGTH * 2 do
		...
		if ( i > ((DUSK_END + DUSK_START) * .5) or i < DAWN_START ) then
			xPitch = 90;
			xYaw = 90;
			xRoll = 0;
		end
...
function GM.progressTime ( )
	...
	if ( GAMEMODE.timeEntities.light_environment and GAMEMODE.timeEntities.pattern != pattern ) then
		local light;
		for _ , light in pairs( GAMEMODE.timeEntities.light_environment ) do
	...
	if ( GAMEMODE.timeEntities.env_sun and GAMEMODE.timeEntities.env_sun_angle != env_sun_angle ) then
		local sun;
		for _ , sun in pairs( GAMEMODE.timeEntities.env_sun ) do
