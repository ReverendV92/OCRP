	...
	for _,ply in pairs(team.GetPlayers(CLASS_CHIEF)) do
		if ply:IsValid() then
			umsg.Start("OCRP_PoliceUpdate" , ply)
				...
				umsg.Long(GAMEMODE.Police_Maxes["item_ammo_9x19mm"])
				umsg.Long(GAMEMODE.Police_Maxes["item_ammo_12gauge"])
				umsg.Long(GAMEMODE.Police_Maxes["item_ammo_556x45"])
			umsg.End()		
		end
	end
end
...
function PMETA:SwatReply(bool,weapon)
	...
	if weapon == 0 then
		self:SwatUpgrade("v92_heat_m3s90")
		self:GiveItem("item_ammo_12gauge" ,8)
	else
		self:SwatUpgrade("swat_ump45_ocrp")
		self:GiveItem("item_ammo_556x45" ,25)
	end
end
...
net.Receive("OCRP_Hire_Police_Chief" , function(len, ply)
	for _,v in pairs(player.GetAll()) do 
		if v:Nick() == net.ReadString() and v:Team() == CLASS_POLICE and Mayor_HasMoney(250) then
			v.DueToBeChief = true
			Mayor_TakeMoney(250)
			OCRP_Job_Quit(v)
