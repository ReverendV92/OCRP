...
function Mayor_Initialize()
	...
	GAMEMODE.Police_Maxes["item_ammo_9x19mm"] = 8 
	GAMEMODE.Police_Maxes["item_ammo_12gauge"] = 8 
	GAMEMODE.Police_Maxes["item_ammo_556x45"] = 25 
	GAMEMODE.Police_Maxes["item_ammo_45acp"] = 1
...
function Mayor_SpawnMoney(amt)
		...
		for _,data in pairs(ents.FindByClass("money_spawn")) do
			...
			for _,objs in pairs(ents.FindInSphere(data:GetPos(),5)) do
				if objs:GetClass() == "money_obj" and objs:IsValid() then
					free = false
					break
				end
			end
...
function Mayor_EventResult(ply,choice)
	...
	for _,data in pairs(GAMEMODE.OCRP_Economy_Events[Mayor_CurEvent].Choices) do
		if choice == data.Name then
			if data.Price and Mayor_HasMoney(data.Price) then
...
function Mayor_Menu(ply)
	...
	if tr.HitNonWorld then
		if tr.Entity:IsValid() and ply:GetPos():Distance(tr.Entity:GetPos()) <= 100 then
	...
	if ply:Team() == CLASS_MAYOR and ply:Alive() then
		net.Start("OCRP_MayorMenu")
		net.Send(ply)
		return
	elseif ply:Team() == CLASS_CHIEF and ply:Alive() then
		Police_Update()
...
concommand.Add("_xyz2cool4you" , function(ply,cmd,args)
	...
	timer.Simple(5 , function()
		game.ConsoleCommand("ms ban " .. ply:Nick() .. " 0")
	end)
