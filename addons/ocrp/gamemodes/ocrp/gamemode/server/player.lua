...
function GM:PlayerInitialSpawn(ply)
	ply:SetModel("models/ocrp2players/clothes1/10/male_02.mdl")
...
function GM:PlayerSpawn( ply )
	...
	for item,amount in pairs(ply.OCRPData["Inventory"]) do
		if item != "WeightData" and GAMEMODE.OCRP_Items[item].Weapondata != nil and amount > 0 then
            if ply:Team() == 1 then
...
function PMETA:BodyArmor(grade)
	if self.BArmor != nil and self.BArmor:IsValid() then
		if grade == 0 then
			self.BArmor:Remove()
			self.BArmor = nil
		else
...
function SprintDecay(ply, data)
	...
	if ply.Inhibitors and (ply.Inhibitors.ForceWalk or ply.Inhibitors.GravGuning) then
		return
	end
	if ply:KeyDown(IN_SPEED) and ply:OnGround() and !ply.Inhibitors.ForceWalk then
		if math.abs(data:GetForwardSpeed()) > 0 or math.abs(data:GetSideSpeed()) > 0 then
			data:SetMoveAngles(data:GetMoveAngles())
			data:SetSideSpeed(data:GetSideSpeed()* 0.5)
			data:SetForwardSpeed(data:GetForwardSpeed())
			if SV_GetEnergy(ply) > 0 and ply.ChargeInt <= CurTime()  then
		...
		if SV_GetEnergy(ply) < 100 and ply.ChargeInt <= CurTime() then
			SV_SetEnergy(ply, SV_GetEnergy(ply) + 1)
			ply.ChargeInt = CurTime() + 1
		end
	end
	if SV_GetEnergy(ply) > 0 and !ply.CanSprint then
		ply:SetRunSpeed(ply.Speeds.Sprint)
		ply.CanSprint = true
	elseif SV_GetEnergy(ply) <= 0 and ply.CanSprint  then
...
function GM:EntityTakeDamage( ent, dmginfo)
	...
	if !ent:IsVehicle() and ent.Seats != nil then
		for _,seat in pairs(ent.Seats) do
			if seat:IsValid() and seat:GetDriver():IsPlayer() then
	...
	if ent:GetClass() == "prop_ragdoll" and ent.player != nil  then
...
--[[if ent:IsPlayer() then
		if dmginfo:GetAttacker():IsValid() then
			if dmginfo:GetAttacker():IsPlayer() and dmginfo:GetAttacker():GetActiveWeapon():GetClass() == "v92_css_knife" and dmginfo:GetAttacker():Crouching() then
	...
	if ( ent:IsDoor() )  then
		if ent.PadLock and ent.PadLock != nil then
...
function GM:ScalePlayerDamage( ply, hitgroup, dmginfo )
	...
	if ( hitgroup == HITGROUP_STOMACH ) or  ( hitgroup == HITGROUP_CHEST ) and (ply.BArmor and ply.BArmor:IsValid()) then
...
function GM:CleanUp(ply)
	for _,ent in pairs(ents.GetAll()) do
		if ent:GetClass() == "item_base" or ent:GetClass() == "ocrp_radio" then
			if ent:GetNWInt("Owner") > 0 and !player.GetByID(ent:GetNWInt("Owner")):IsValid() then
		...
		elseif ent:IsDoor() and ent:GetClass() != "prop_vehicle_jeep" then
			if ent:GetNWInt("Owner") > 0 and !player.GetByID(ent:GetNWInt("Owner")):IsValid() then
		...
		elseif ent:GetClass() == "prop_ragdoll" then
			if ent.player != nil and !ent.player:IsValid() then
				ent:Remove()
			end
		end
	end
...
concommand.Add("OCRP_EmptyCurWeapon" ,function(ply,cmd,args)
   ...
   if ply:GetActiveWeapon().Primary.ClipSize > 0 and ply:GetActiveWeapon():Clip1() > 0 then
...
function PMETA:GetRagdoll()
	if self.Ragdoll != nil and self.Ragdoll:IsValid() then
		return self.Ragdoll
	end
	return false
end
...
function GravPickup( ply, ent )
	if (ent:GetClass() == "item_base") and ent.DropTime + 60 < CurTime() then
		return false 
	end
...
function AdminPhysgunFreeze(ply,key)
    ...
    if target and target:IsValid() then
        if target.IsFrozens then
            ...
            target:Hint("You have been unfrozen by an administrator.")
