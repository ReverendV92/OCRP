...
local matLogo = Material("jessev92/ui/header_cult.png")
...
function GM:HUDShouldDraw(Name)
	if INTRO and Name != "CHudGMod" then
		return false
	end
	...
	if LocalPlayer():GetNWBool("Handcuffed") and Name == "CHudWeaponSelection" then
		return false
	end
	return true
end
...
local Messages = {"http://steamcommunity.com/groups/CultOfV92 :: https://discord.gg/QQmCerH"}
...
function drawSpeedLimits(isDepth, isSkybox)
	...
	if game.GetMap() == "rp_rockford_v2b" then
		...
		local pos2 = Vector( -13383 , 2618 , 627 )
		pos2.z = 272 + bounce
		local cost = round(1.25*(1+(GetGlobalInt("Eco_Tax")/100)), 2)
		cam.Start3D2D(pos2 , Angle(0 ,90 ,90), 1)
			draw.DrawText("Gas Price: $" .. cost .. "/L" , "SpeedLimit" , 0 , 0 , Color(177 ,28 ,28 ,255), 1 , 1)
		cam.End3D2D()
...
function GM:HUDPaint()
	...
	if client:Alive() or OCRP_DeathInfo.SpawnTime == nil then 
		...
		if (LocalPlayer():Team() != CLASS_CITIZEN and LocalPlayer():Team() != CLASS_MAYOR and LocalPlayer():GetNWBool("tradio" , false)) then
			surface.SetDrawColor(255 , 255 , 255 , 255);
		...
		if LocalPlayer():GetNWBool("Handcuffed") then
			...
			if OCRP_ArrestData != nil and OCRP_ArrestData.Time >= CurTime() then
				surface.SetTextColor(Color(200 ,50 ,50))
				surface.SetFont("Trebuchet24")
	...
	--	draw.RoundedBox(10 ,SW - 200 ,SH -80 ,150 ,70 ,Color( 210 , 120 , 30 , 255 ))
		if LocalPlayer():GetActiveWeapon():IsValid() and (LocalPlayer():GetActiveWeapon():Clip1() >= 0 or LocalPlayer():GetActiveWeapon().PrintName == "Police Taser") and LocalPlayer():GetActiveWeapon():GetClass() != "weapon_physcannon" then
       ...
       -- surface.DrawTexturedRect(10 ,65 ,50 ,50)
		...
		if OCRP_Cur911Alert.Message != nil and LocalPlayer():Team() != CLASS_CITIZEN then
		...
		if LocalPlayer():InVehicle() and !LocalPlayer():GetVehicle():GetParent():IsValid() then
			surface.SetDrawColor(255 ,255 ,255 ,255)
			surface.SetMaterial(OC_HpBar)
...
function GM:HUDDrawTargetID()
	...
	if LocalPlayer():InVehicle() then  
		if LocalPlayer():Team() == CLASS_POLICE or LocalPlayer():Team() == CLASS_CHIEF then
			for _,ent in pairs(ents.FindByClass("prop_vehicle_jeep")) do
				if ent:GetPos():Distance(LocalPlayer():GetPos()) <= 700 and LocalPlayer():GetVehicle() != ent then
		...
		if trace.Hit and trace.Entity and (trace.Entity:IsDoor() or trace.Entity:GetClass() == "prop_vehicle_jeep" ) and trace.Entity:GetPos():Distance(OurPos) < 200 then	
			if trace.Entity:IsDoor() and GAMEMODE.Maps[string.lower(game.GetMap())] != nil then
				...
				if trace.Entity:GetNWInt( "Owner" ) > 0 and player.GetByID(trace.Entity:GetNWInt("Owner")):IsValid() then
					...
					--if trace.Entity:IsVehicle() and trace.Entity:GetNWEntity("Driver"):IsValid() then
				...
				elseif unownable == -3 then
					...
					if propkey and GAMEMODE.Properties[string.lower(game.GetMap())][propkey] != nil then 
		...
		for _,ent in pairs(OCRP_ShopItems) do
			if ent:IsValid() and ent.cl_price != nil and ent.cl_desc != nil then
				...
				if PLY:GetObserverMode() != 4 and PLY:GetObserverMode() != 5 and a != 0 then
					if !TRACE.Hit then
						if Distance <= 1600 then  
							...
							if LocalPlayer():Team() == CLASS_POLICE or  LocalPlayer():Team() == CLASS_CHIEF or  LocalPlayer():Team() == CLASS_SWAT or LocalPlayer():Team()  == CLASS_MAYOR then
